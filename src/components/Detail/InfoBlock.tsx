    ...
    >
      <Flex style={{ flexDirection: "column", rowGap: "10px" }}>
        ...
        <Flex className={styles.CustomUtilsFlex}>
          <Flex gap={4} className={styles.CustomFlexInfoBlock}>
            <Star />
            {product.rating}
          </Flex>
          <Flex justify={"center"} className={styles.CustomFlexInfoBlock}>
            <img width={100} src={`${API_URL}${product.brand_image}`} />
          </Flex>
          <Button
            appearance="lightBlue"
            className={`${styles.CustomButtonInfoBlock} ${
              isProductInFavorites ? styles.clickedHeart : styles
            }`}
            onClick={() => handleClickFavorite(product.id)}
          >
            <HeartOutlined
              style={{
                display: `${isProductInFavorites ? "none" : "initial"}`,
              }}
            />
